import * as tui from "./tantalum-ui";
...
export class Tantalum {
    ...
    colorBufferFloatTest(gl: WebGL2RenderingContext) {
        /* This one is slightly awkward. The WEBGL_color_buffer_float
           ...
           Hurray WebGL! */
        const shader = new tgl.Shader(gl, Shaders, "blend_test_vert", "blend_test_frag");
        const packShader = new tgl.Shader(gl, Shaders, "blend_test_vert", "blend_test_pack_frag");
        const target = new tgl.Texture(gl, 1, 1, 4, true, false, false, new Float32Array([-6.0, 10.0, 30.0, 2.0]));
        const fbo = new tgl.RenderTarget(gl, this.multiBufExt);
        const vbo = new tgl.VertexBuffer(gl);
        vbo.bind();
